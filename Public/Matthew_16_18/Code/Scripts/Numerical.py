# ---------------------------------
# Script Name: Numerical.py
# Version: 1.0.0.0
# Description: The function sort_input_numerically organizes entries from a specified blocklist file into separate numerically sorted text files based on their starting letters.
# Author: ThePerfectWill
# Usage: python3 Code/Scripts/Numerical.py
...
def sort_input_numerically(input_file: str, output_dir: Path) -> None:
    ...
    try:
        ...
        entries_by_digit = defaultdict(list)

        for line in lines:
            if line:  # Ensure the line is not empty
                first_char = line[0]
                if first_char.isdigit() and '0' <= first_char <= '9':
                    entries_by_digit[first_char].append(line)

        for digit, entries in entries_by_digit.items():
            with open(output_dir / f"{digit}.txt", "w") as f:
                f.write("\n".join(sorted(entries)))
                logging.info(f"Compiled {len(entries)} entries starting with digit '{digit}' to '{output_dir}/{digit}.txt'")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Sort entries from a blocklist into numerical files.')
    parser.add_argument('--i', '--input_file', type=str, default=BLOCKLIST_TXT, help='Input blocklist file (default: blocklist.txt)')
    parser.add_argument('--o', '--output_file', type=str, default=str(DEFAULT_OUTPUT_DIR), help='Output directory for sorted files (default: Numerical in SORTED_PATH)')
    ...
    sort_input_numerically(args.i, Path(args.o))
