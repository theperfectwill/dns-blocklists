...
FINAL_JSON = Path(Data.FINAL_PATH) / "regexDNSF.json"
FINAL_TXT = Path(Data.FINAL_PATH) / "regexDNSF.txt"
...
def reformat_final_file(file_path):
    try:
        ...
        # Get the line count of Regex/Rules/combined.txt
        with open(COMBINED_TXT, 'r') as source_file:
            combined_rule_count = len(source_file.readlines())

        # Get the line count of the file minus the file_comments count (currently 12)
        with open(file_path, 'r') as file:
            lines = file.readlines()
            current_rule_count = max(0, len(lines) - 15)
        ...
        file_comments = (
            ...
            f"{Data.RULE_REDUCED_COUNT_COMMENT} We reduced {reduced_rule_count} rules with {combined_rule_count} wildcard/regex rules.\n"
